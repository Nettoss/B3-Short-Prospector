...
# FIX 1: Função robusta de download que normaliza colunas independente da versão do yfinance
...
def baixar_dados(ticker_sa: str, period: str) -> pd.DataFrame:
    try:
        ...
        if df.empty:
            return pd.DataFrame()
        # Normaliza MultiIndex (presente em algumas versões do yfinance)
...
with st.sidebar:
    ...
    st.markdown('<div class="header-sub">Prospector v1.1</div>', unsafe_allow_html=True)
...
with aba1:
    ...
    # FIX 2: Substituído st.stop() por st.error() + bloco condicional
    # st.stop() parava a execução global, impedindo a aba 2 de carregar
    if df.empty:
        st.error(f"❌ Não foi possível obter dados para **{ticker_input}**. Verifique o ticker ou tente novamente.")
    else:
        close = df["Close"]
        volume = df["Volume"]
...
with aba2:
    ...
    if df_rank.empty:
        ...
        # FIX 3: ProgressColumn — format corrigido para aceitar floats sem string formatting
        # O format="%f" causava erro pois a coluna Score Short é numérica
        st.dataframe(
            df_display,
            ...
            column_config={
                "Score Short": st.column_config.ProgressColumn(
                    "Score Short", min_value=0, max_value=100, format="%.1f"
                ),
